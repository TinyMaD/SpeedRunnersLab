...
**更新** 新增完整的自动化部署脚本和生产环境配置，提供 ECS 服务器初始化和本地开发环境配置流程。
...
**章节来源**
- [docker-compose.prod.yml:12-72](file://docker-compose.prod.yml#L12-L72)
...
- ACR 信息收集：收集阿里云 ACR 配置信息。
- GitHub Secrets 管理：自动写入或指导手动配置 Secrets。

**章节来源**
- [scripts/setup-local.ps1:1-214](file://scripts/setup-local.ps1#L1-L214)
...
- GitHub Actions 依赖阿里云 ACR 和 ECS 服务器。
...
NET --- SCHED
NET --- DB
GHA["GitHub Actions"] --> ACR["阿里云 ACR"]
ECS["ECS 服务器"] --> UI
ECS --> API
ECS --> SCHED
```
...
**章节来源**
- [DEPLOY.md:129-132](file://DEPLOY.md#L129-L132)
...
- [DEPLOY.md:98-128](file://DEPLOY.md#L98-L128)
- [scripts/setup-ecs.sh:114-136](file://scripts/setup-ecs.sh#L114-L136)

## 结论
本方案采用 docker-compose 将前端、后端、调度器与数据库解耦编排，配合 Nginx 提供反向代理与静态资源服务。通过 GitHub Actions 实现自动化 CI/CD，支持一键部署到 ECS 服务器。生产环境建议补充 secrets 管理、集中日志、健康检查、负载均衡与多副本部署，并完善数据库备份与版本升级回滚流程，以满足高可用与可维护性需求。

**更新** 新增完整的自动化部署脚本和生产环境配置，提供 ECS 服务器初始化和本地开发环境配置流程，大大简化了部署复杂度和运维成本。
...
**章节来源**
- [DEPLOY.md:87-95](file://DEPLOY.md#L87-L95)
...
**章节来源**
- [scripts/setup-local.ps1:148-155](file://scripts/setup-local.ps1#L148-L155)
